...
def classify_url(path_segments):
    # City: /local-agent/<state>/<city>/
    # Agency: /local-agent/<state>/<city>/<agency>/
    if len(path_segments) == 3 and path_segments[0] == "local-agent":
        return "city"
    if len(path_segments) == 4 and path_segments[0] == "local-agent":
        return "agency"
    return None
...
def save_states_list():
    ...
    states_path = os.path.join(PROGRESSIVE_DIR, "states.txt")
...
def retry_failed_agents(state):
    ...
    if city_urls:
        ...
        for city_url in city_urls:
            agents_added = process_city_for_state(city_url, output_path, error_path)
